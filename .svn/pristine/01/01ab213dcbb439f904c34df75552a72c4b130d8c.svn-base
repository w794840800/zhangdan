 ...
 */
public class WebActivity extends MvpActivity<CommonPresenter> implements CommonView<FundDetailModel>,
        OtherView<FundDetailModel>,ThirdView<FundDetailModel> {
    @Bind(R.id.tv_title)
    TextView tvTitle;
    @Bind(R.id.ib_back)
    ImageButton ibBack;
...
//        ActivityUtils.setStatusBar(mActivity);
    ...
    public class WebViewClient extends android.webkit.WebViewClient {
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            LogUtils.logd("跳转url："+url);
            // 调用拨号程序
            if (url.startsWith("en://StockRecord")) {//股票操作记录列表
                StatService.onEvent(mActivity, "股票详情页点击操作记录入口", "查看股票操作记录", 1);
