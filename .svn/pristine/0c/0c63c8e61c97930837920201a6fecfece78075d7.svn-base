...
public class AddInsuranceAssets extends MvpActivity<CommonPresenter> implements CommonView<SercetKeyOverdueModel> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity, "点击保险资产编辑页返回按钮", "放弃保险资产编辑", 1);
    ...
    private void handleIntent(Intent in) {
        ...
        if(memo != null && !TextUtils.isEmpty(memo) || yearBonus != null && !TextUtils.isEmpty(yearBonus)){
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.llShowMore:
                StatService.onEvent(mActivity, "添加保险资产时点击更多", "添加更多保险信息", 1);
                ImeUtil.hideSoftKeyboard(cetProductName);
                if (!isMoreShow) {
                    valueAnimatorShow.start();
                    isMoreShow = true;
            ...
            case R.id.btnSave:
                if (isEdit) {
                    StatService.onEvent(mActivity, "点击保险资产编辑页确认按钮", "编辑保险资产", 1);
                } else if (isFromAddasset) {
                    StatService.onEvent(mActivity, "点击保险资产添加页确认按钮", "首次新增保险", 1);
                } else {
                    StatService.onEvent(mActivity, "保险列表进入添加页并点击确认按钮", "确认新增保险", 1);
                }
                if (valid()) {
                    sava();
                }
    ...
    protected void onResume() {
        super.onResume();
        if (isEdit) {
            StatService.onPageStart(mActivity, "编辑保险页面");
        } else {
            StatService.onPageStart(mActivity, "添加保险页面");
        }
    }
    ...
    protected void onPause() {
        ...
        if (isEdit) {
            StatService.onPageEnd(mActivity, "编辑保险页面");
        } else {
            StatService.onPageEnd(mActivity, "添加保险页面");
        }
    }
