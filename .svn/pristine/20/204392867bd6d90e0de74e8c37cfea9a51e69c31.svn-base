...
import com.beidou.ybz.accountbook.adapter.AdapterCustom;
import com.beidou.ybz.accountbook.adapter.AdapterPrivateOffering;
 ...
 * module: 私募基金界面
 */
public class PrivateOfferingActivity extends MvpActivity<CommonPresenter> implements CommonView<PrivateOfferingModel>, OtherView<SercetKeyOverdueModel> {
    ...
    private List<PrivateOfferingModel.BodyBean.PositionListBean> proListBeanList;
    private AdapterPrivateOffering mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvAddAssets.setText("+  新建私募");
        tvTitle.setText("私募");
        tvMingcheng.setText("当前市值(元)");
        ...
        mAdapter = new AdapterPrivateOffering(R.layout.overseaslist_item, null);
        rv.setAdapter(mAdapter);

        tvName.setText("产品名称");
        tvValue.setText("当前市值");
        ...
        mAdapter.setOnItemLongClickListener(new BaseQuickAdapter.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(BaseQuickAdapter adapter, View view, int position) {
                alertDialogUtils.setMessage("确定要删除" + proListBeanList.get(position).getFundName() + "？");
...
//                Intent in = new Intent(mActivity, CustomDetailActivity.class);
//                in.putExtra("id", proListBeanList.get(position).getId());
//                in.putExtra("name", proListBeanList.get(position).getFundName());
//                startActivity(in);
//                overridePendingTransition(R.anim.left_in, 0);
                String id = proListBeanList.get(position).getId();//股票id
                String fundName = proListBeanList.get(position).getFundName();//股票名称
                String fundCode = proListBeanList.get(position).getFundCode();//股票代码
                String positionNumber = proListBeanList.get(position).getPositionNumber();//持仓数量
                String brokerName = proListBeanList.get(position).getBrokerName();//券商名称
                String memo = proListBeanList.get(position).getMemo();//备注内容
                requestUrl(id,fundCode,fundName,positionNumber,brokerName,memo);
            }
        });
        ...
        alertDialogUtils.setOnDialogClickListener(new DialogClickListener() {
            ...
            public void clickYes(int which) {
                StatService.onEvent(mActivity,"点击私募列表任一删除按钮","删除私募具体资产",1);
    ...
    protected void onResume() {
        super.onResume();
        request();
        StatService.onPageStart(mActivity,"私募列表页面");
    }

    @Override
    protected void onPause() {
        super.onPause();
        StatService.onPageEnd(mActivity,"私募列表页面");
    }
    ...
    void request() {
        ...
        mvpPresenter.getsmfundlist(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        if (count == 0) {
            mEmptyView.loading();
        }
    }
    ...
    void requestUrl(String id,String stockCode,String stockName,String positionNumber,String brokerName,String memo) {
        ...
        String url = ApiStores.SimuDetail+"?encMsg="+encMsg+"&signMsg="+signMsg+"&msgType=2"+"&secretKeyId="+spUtils.getSecretKeyId();
        ...
        in.putExtra("from","smfund");//来自私募基金列表
        in.putExtra("id",id);
        in.putExtra("url",url);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tvAddAssets:
                StatService.onEvent(mActivity,"私募列表页点击新增","新增私募",1);
                Intent in = new Intent(mActivity, AddPrivateOfferingFund.class);
    ...
    @Override
    public void getDataSuccess(PrivateOfferingModel model) {
        count++;
        try {
            mSwipeRefreshLayout.setRefreshing(false);
            proListBeanList = model.getBody().getPositionList();
            ...
            if (proListBeanList != null && proListBeanList.size() > 0) {
                ...
                mEmptyView.setEmptyText("还没有私募基金产品哦～");
                mEmptyView.empty();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    void del(String id) {
        ...
        mvpPresenter.delsmfund(encMsg, signMsg, "2", spUtils.getSecretKeyId());
    }
    ...
    public void onSuccess(SercetKeyOverdueModel model) {
        ...
        try {
            zzzj = String.valueOf(new BigDecimal(Double.parseDouble(zzzj) - Double.parseDouble(proListBeanList.get(mPosition).getDqsz())));
            tvZczj.setText(Utils.addCommaContainsPoint(zzzj));
