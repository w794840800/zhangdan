...
import com.beidou.ybz.accountbook.adapter.AdapterCurrentfinancial;
 ...
 * module: 活期理财界面
 */
public class CurrentFinancialActivity extends MvpActivity<CommonPresenter> implements CommonView<CurrentFinancialListModel>, OtherView<SercetKeyOverdueModel> {
    ...
    private List<CurrentFinancialListModel.BodyBean.ProListBean> proListBeanList;
    private AdapterCurrentfinancial mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvAddAssets.setText("+  新建活期理财");
        tvTitle.setText("活期理财");
        ...
        mAdapter = new AdapterCurrentfinancial(R.layout.overseaslist_item, null);
        ...
        mAdapter.setOnItemLongClickListener(new BaseQuickAdapter.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(BaseQuickAdapter adapter, View view, int position) {
                if(proListBeanList.get(position).getCurrentType() != null && proListBeanList.get(position).getCurrentType().equals("6")) {//活期理财
        ...
        mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                if(proListBeanList.get(position).getCurrentType() != null && proListBeanList.get(position).getCurrentType().equals("6")){//活期理财
                    ...
                    Intent in = new Intent(mActivity, CurrentFinancialDetailActivity.class);
        ...
        alertDialogUtils.setOnDialogClickListener(new DialogClickListener() {
            ...
            public void clickYes(int which) {
                StatService.onEvent(mActivity,"点击活期理财列表任一删除按钮","删除活期理财具体资产",1);
    ...
    void requestUrl() {
        ...
        Intent in = new Intent(mActivity, X5WebActivity.class);
        in.putExtra("url", url);
        startActivity(in);
        overridePendingTransition(R.anim.left_in, 0);
    }
    ...
    protected void onResume() {
        super.onResume();
        request();
        StatService.onPageStart(mActivity, "活期理财列表页面");
    }

    @Override
    protected void onPause() {
        super.onPause();
        StatService.onPageEnd(mActivity, "活期理财列表页面");
    }
    ...
    void request() {
        ...
        mvpPresenter.getcurfinlist(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        if (count == 0) {
            mEmptyView.loading();
        }
    }
...
//                }
            ...
            case R.id.tvAddAssets:
                StatService.onEvent(mActivity,"活期理财列表页点击新增","新增活期理财",1);
                Intent in = new Intent(mActivity, AddCurrentFinancial.class);
                in.putExtra("from", "list");//currentTypeList
//                    in.putExtra("currentTypeList", currentTypeList);
                in.putStringArrayListExtra("currentTypeList", currentTypeList);
                startActivity(in);
                overridePendingTransition(R.anim.left_in, 0);
//                }
                break;
    ...
    @Override
    public void getDataSuccess(CurrentFinancialListModel model) {
        count++;
        try {
            ...
            if (proListBeanList != null && proListBeanList.size() > 0) {
                ...
                int size = proListBeanList.size();

                for (int i = 0; i < size; i++) {
                    currentTypeList.add(proListBeanList.get(i).getCurrentType());
                }

            } else {
                mEmptyView.setEmptyText("还没有活期理财产品哦～");
                mEmptyView.empty();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    void del(String id) {
        ...
        mvpPresenter.delcurfin(encMsg, signMsg, "2", spUtils.getSecretKeyId());
    }
