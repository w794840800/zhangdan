...
import com.beidou.ybz.accountbook.adapter.AdapterArrears;
 ...
 * module: 欠款界面
 */
public class ArrearsassetActivity extends MvpActivity<CommonPresenter> implements CommonView<ArrearsModel>, OtherView<SercetKeyOverdueModel> {
    ...
    private List<ArrearsModel.BodyBean.ProListBean> proListBeanList;
    private AdapterArrears mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvAddAssets.setText("+  新建欠款");
        tvTitle.setText("欠款");
        tvMingcheng.setText("欠款总计(元)");
        tvName.setText("欠款人");
        tvValue.setText("欠款金额");
        ...
        mAdapter = new AdapterArrears(R.layout.arrearslist_item, null);
        ...
        mAdapter.setOnItemLongClickListener(new BaseQuickAdapter.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(BaseQuickAdapter adapter, View view, int position) {
                alertDialogUtils.setMessage("确定要删除" + proListBeanList.get(position).getArrearsName() + "？");
        ...
        mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                Intent in = new Intent(mActivity, ArrearsDetailActivity.class);
                in.putExtra("id", proListBeanList.get(position).getId());
                in.putExtra("name", proListBeanList.get(position).getArrearsName());
                startActivity(in);
        ...
        alertDialogUtils.setOnDialogClickListener(new DialogClickListener() {
            ...
            public void clickYes(int which) {
                StatService.onEvent(mActivity,"点击欠款列表任一删除按钮","删除欠款具体资产",1);
    ...
    protected void onResume() {
        super.onResume();
        request();
        StatService.onPageStart(mActivity, "欠款列表页面");
    }

    @Override
    protected void onPause() {
        super.onPause();
        StatService.onPageEnd(mActivity, "欠款列表页面");
    }
    ...
    void request() {
        ...
        mvpPresenter.getArrearsList(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        if (count == 0) {
            mEmptyView.loading();
        }
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tvAddAssets:
                StatService.onEvent(mActivity,"欠款列表页点击新增","新增欠款",1);
//                ActivityUtils.startActivityRightIn(mActivity, AddCurrentFinancial.class);
                Intent in = new Intent(mActivity, AddArrears.class);
    ...
    @Override
    public void getDataSuccess(ArrearsModel model) {
        count++;
        try {
            ...
            if (proListBeanList != null && proListBeanList.size() > 0) {
                ...
                mEmptyView.setEmptyText("还没有欠款产品哦～");
                mEmptyView.empty();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    void del(String id) {
        ...
        mvpPresenter.delarrearsasset(encMsg, signMsg, "2", spUtils.getSecretKeyId());
    }
    ...
    public void onSuccess(SercetKeyOverdueModel model) {
        ...
        try {
//            zzzj = String.valueOf(Double.parseDouble(zzzj) - Double.parseDouble(proListBeanList.get(mPosition).getAmount()));
//            tvZczj.setText(Utils.addCommaContainsPoint(zzzj));
            request();
            proListBeanList.remove(mPosition);
            mAdapter.notifyItemRemoved(mPosition);//推荐用这个
            if (mAdapter.getItemCount() == 0) {
                mEmptyView.setEmptyText("暂无产品");
                mEmptyView.empty();
            }
        } catch (Exception e) {
            e.printStackTrace();
