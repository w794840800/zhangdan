...
public class AddBankCard extends MvpActivity<CommonPresenter> implements CommonView<SercetKeyOverdueModel> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity,"点击银行卡资产编辑页返回按钮","放弃银行卡资产编辑",1);
    ...
    private void handleIntent(Intent in){
        ...
        if(memo != null && !TextUtils.isEmpty(memo) || bankNo != null && !TextUtils.isEmpty(bankNo)){
            tvCardname.postDelayed(new Runnable() {
                @Override
                public void run() {
        ...
        if(amount != null && !TextUtils.isEmpty(amount) || bankType != null && bankType.equals("2")){//信用卡
            tvCardname.postDelayed(new Runnable() {
                @Override
                public void run() {
...
//                ActivityUtils.startActivityRightIn(mActivity,SupportBankActivity.class);
            ...
            case R.id.llShowMore:
                StatService.onEvent(mActivity,"添加银行卡资产时点击更多","添加更多银行卡信息",1);
                ImeUtil.hideSoftKeyboard(view);
                if (!isMoreShow) {
                    valueAnimatorMoreShow.start();
                    isMoreShow = true;
            ...
            case R.id.btnSave:
                if(isEdit){
                    StatService.onEvent(mActivity,"点击银行卡资产编辑页确认按钮","编辑银行卡资产",1);
                }else if(isFromAddasset){
                    StatService.onEvent(mActivity,"点击银行卡资产添加页确认按钮","首次新增银行卡",1);
                }else{
                    StatService.onEvent(mActivity,"银行卡列表进入添加页并点击确认按钮","确认新增银行卡",1);
                }
                if (valid()) {
                    ImeUtil.hideSoftKeyboard(view);
                    sava();
...
//        in.putExtra("tag", tag);
    ...
    protected void onResume() {
        super.onResume();
        if(isEdit) {
            StatService.onPageStart(mActivity, "编辑银行卡页面");
        }else{
            StatService.onPageStart(mActivity, "添加银行卡页面");
        }
    }
    ...
    protected void onPause() {
        ...
        ImeUtil.hideSoftKeyboard(tvCardname);
        if(isEdit) {
            StatService.onPageEnd(mActivity, "编辑银行卡页面");
        }else{
            StatService.onPageEnd(mActivity, "添加银行卡页面");
        }
    }
