...
public class AddCurrentFinancial extends MvpActivity<CommonPresenter> implements CommonView<SercetKeyOverdueModel>, TextWatchView {
    ...
    @Bind(R.id.llZhifubao) LinearLayout llZhifubao;
    @Bind(R.id.llLicaitong) LinearLayout llLicaitong;
    @Bind(R.id.llWeizhong) LinearLayout llWeizhong;
    @Bind(R.id.llJindongjinku) LinearLayout llJindongjinku;
    @Bind(R.id.llQita) LinearLayout llQita;
    @Bind(R.id.cetProductName) ClearEditText cetProductName;
    @Bind(R.id.cetAmount) ClearEditText cetAmount;
    @Bind(R.id.btnSave) Button btnSave;
    @Bind(R.id.ll1) LinearLayout ll1;
    @Bind(R.id.ll2) LinearLayout ll2;
    @Bind(R.id.rvCurrent) RecyclerView rvCurrent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity,"点击活期理财资产编辑页返回按钮","放弃活期理财资产编辑",1);
    ...
    public void getDataSuccess(SercetKeyOverdueModel model) {
        if (isEdit) {
            ...
            cetProductName.postDelayed(new Runnable() {
                @Override
                public void run() {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnSave:
                if(isEdit){
                    StatService.onEvent(mActivity,"点击活期理财资产编辑页确认按钮","编辑活期理财资产",1);
                }else if(isFromAddasset){
                    StatService.onEvent(mActivity,"点击活期理财资产添加页确认按钮","首次新增活期理财",1);
                }else{
                    StatService.onEvent(mActivity,"活期理财列表进入添加页并点击确认按钮","确认新增活期理财",1);
                }
    ...
    protected void onResume() {
        super.onResume();
        if(isEdit) {
            StatService.onPageStart(mActivity, "编辑活期理财页面");
        }else{
            StatService.onPageStart(mActivity, "添加活期理财页面");
        }
    }
    ...
    protected void onPause() {
        ...
        ImeUtil.hideSoftKeyboard(cetProductName);
        if(isEdit) {
            StatService.onPageEnd(mActivity, "编辑活期理财页面");
        }else{
            StatService.onPageEnd(mActivity, "添加活期理财页面");
        }
    }
