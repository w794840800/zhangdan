...
public class ClearEditText extends AppCompatEditText {
    ...
    //清除按钮宽度,单位DP
    private final int WIDTH_OF_CLEAR = 20;
    ...
    //是否有文字填充
    private boolean isTextAvail = false;
    private boolean isClearButtonAvail = false;
    //右边添加其他按钮时使用
...
//        setSingleLine();
    ...
    protected void drawClearGone(float scale, Canvas canvas) {
        isClearButtonAvail = false;
        LogUtils.logd("输入框isClearButtonAvail:置为false"+isClearButtonAvail);
...
//        LogUtils.logd(text + "--" + start + "--" + lengthBefore + "--" + lengthAfter);
//        LogUtils.logd("hasFocus()" + hasFocus() + "--" + hasFocusable() );
        if (text.length() > 0) {
            isTextAvail = true;
        }
        if (hasFocus()) {
    ...
    protected void onFocusChanged(boolean focused, int direction, Rect previouslyFocusedRect) {
        ...
        LogUtils.logd("onFocusChanged：focused"+focused+"——isTextAvail"+isTextAvail);
        if (focused && isTextAvail) {//获取到焦点且有文字时
            if (!isVisible) {//如果未显示
                isVisible = true;
                startVisibleAnimator();
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            LogUtils.logd("输入框touch:"+touchable+"--"+isClearButtonAvail);
            if (touchable && isClearButtonAvail) {
                setError(null);
                this.setText("");
            }
        }
