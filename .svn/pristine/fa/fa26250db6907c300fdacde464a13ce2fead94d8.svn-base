...
public class OverseasActivity extends MvpActivity<CommonPresenter> implements CommonView<OverseasListModel>, OtherView<SercetKeyOverdueModel> {
    ...
    private String orderNo, productNo,defaultFlag;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter.setOnItemLongClickListener(new BaseQuickAdapter.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(BaseQuickAdapter adapter, View view, int position) {
                defaultFlag = overseasListBeanList.get(position).getDefaultFlag();
                if (defaultFlag != null && defaultFlag.equals("1")) {//本平台的海外资产数据
...
//                ActivityUtils.startActivityRightIn(mActivity,InsuranceDetailActivity.class);
                orderNo = overseasListBeanList.get(position).getOrderNo();
                productNo = overseasListBeanList.get(position).getProductNo();
                defaultFlag = overseasListBeanList.get(position).getDefaultFlag();
                if (defaultFlag != null && defaultFlag.equals("1")) {//本平台的海外资产数据
                    requestUrl(orderNo, productNo);
                } else {
        ...
        alertDialogUtils.setOnDialogClickListener(new DialogClickListener() {
            ...
            public void clickYes(int which) {
                StatService.onEvent(mActivity, "点击房产列表任一删除按钮", "删除房产具体资产", 1);
...
//        request();
    ...
    //房产详情首页
    void requestUrl(String orderNo, String productNo) {
        ...
        requestModel.setOrderNo(orderNo);
        requestModel.setProductNo(productNo);
        ...
        String url = ApiStores.houseorderDetail + "?encMsg=" + encMsg + "&signMsg=" + signMsg + "&msgType=2" + "&secretKeyId=" + spUtils.getSecretKeyId();
        LogUtils.loge("请求url：" + url);
        Intent in = new Intent(mActivity, X5TitleWebActivity.class);
        in.putExtra("url", url);
        startActivity(in);
        overridePendingTransition(R.anim.left_in, 0);
    }
...
//                }
            ...
            case R.id.tvAddAssets:
                StatService.onEvent(mActivity, "房产列表页点击新增", "新增房产", 1);
