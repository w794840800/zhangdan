...
public class AddDomin extends MvpActivity<CommonPresenter> implements CommonView<SercetKeyOverdueModel> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity, "点击域名资产编辑页返回按钮", "放弃域名资产编辑", 1);
    ...
    private void handleIntent(Intent in) {
        ...
        if (memo != null && !TextUtils.isEmpty(memo)
                || buyDate != null && !TextUtils.isEmpty(buyDate)
                || endDate != null && !TextUtils.isEmpty(endDate)) {
...
//        cetMoney.addTextChangedListener(new NewWatcher(this));
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.llShowMore:
                StatService.onEvent(mActivity, "添加域名资产时点击更多", "添加更多域名信息", 1);
                ImeUtil.hideSoftKeyboard(cetName);
                if (!isMoreShow) {
                    valueAnimatorShow.start();
                    isMoreShow = true;
            ...
            case R.id.btnSave:
                if (isEdit) {
                    StatService.onEvent(mActivity, "点击域名资产编辑页确认按钮", "编辑域名资产", 1);
                } else if (isFromAddasset) {
                    StatService.onEvent(mActivity, "点击域名资产添加页确认按钮", "首次新增域名", 1);
                } else {
                    StatService.onEvent(mActivity, "域名列表进入添加页并点击确认按钮", "确认新增域名", 1);
                }
                if (valid()) {
                    sava();
                }
    ...
    protected void onResume() {
        super.onResume();
        if (isEdit) {
            StatService.onPageStart(mActivity, "编辑域名页面");
        } else {
            StatService.onPageStart(mActivity, "添加域名页面");
        }
    }
    ...
    protected void onPause() {
        ...
        if (isEdit) {
            StatService.onPageEnd(mActivity, "编辑域名页面");
        } else {
            StatService.onPageEnd(mActivity, "添加域名页面");
        }
    }
