...
public class AddInternetAccount extends MvpActivity<CommonPresenter> implements CommonView<SercetKeyOverdueModel> {
    ...
    private boolean isFromList;//来自列表
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity,"点击账号资产编辑页返回按钮","放弃账号资产编辑",1);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnSave:
                if(isEdit){
                    StatService.onEvent(mActivity,"点击账号资产编辑页确认按钮","编辑账号资产",1);
                }else if(isFromAddasset){
                    StatService.onEvent(mActivity,"点击账号资产添加页确认按钮","首次新增账号",1);
                }else{
                    StatService.onEvent(mActivity,"账号列表进入添加页并点击确认按钮","确认新增账号",1);
                }
                if (valid()) {
                    save();
                }
