 ...
 * module: 添加借款
 */
public class AddLoan extends MvpActivity<CommonPresenter> implements OtherView<SercetKeyOverdueModel> {
    @Bind(R.id.tv_title)
    TextView tvTitle;
    @Bind(R.id.toolbar)
    Toolbar toolbar;
    ...
    private String id, name ,memo ,amount ,from,loanTime,returnTime,loanStatus,interest;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity,"点击借款资产编辑页返回按钮","放弃借款资产编辑",1);
    ...
    private void handleIntent(Intent in){
        ...
        if (in != null) {
            ...
            name = in.getStringExtra("loanName");
            memo = in.getStringExtra("memo");
            //loanTime,returnTime,loanStatus,interest
            loanTime = in.getStringExtra("loanTime");
            amount = in.getStringExtra("amount");
            returnTime = in.getStringExtra("returnTime");
            loanStatus = in.getStringExtra("loanStatus");
            interest = in.getStringExtra("interest");
            from = in.getStringExtra("from");
        }

        if(memo != null && !TextUtils.isEmpty(memo)
                || loanTime != null && !TextUtils.isEmpty(loanTime)
                || returnTime != null && !TextUtils.isEmpty(returnTime)
                || interest != null && !TextUtils.isEmpty(interest)){
        ...
        if(isEdit){
            tvTitle.setText("编辑借款");
        }else{
            tvTitle.setText("添加借款");
        }
    ...
    private DatePickerDialog.OnDateSetListener datelistener = new DatePickerDialog.OnDateSetListener() {
        ...
        private void updateDate() {
            //在TextView上显示日期
            if (isBorrow) {
                tvBorrowTime.setText(year + "年" + (month + 1) + "月" + day + "日");
            } else {
                tvReturnTime.setText(year + "年" + (month + 1) + "月" + day + "日");
            }
        }
    };
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.llShowMore:
                StatService.onEvent(mActivity,"添加借款资产时点击更多","添加更多借款信息",1);
                ImeUtil.hideSoftKeyboard(cetName);
                if (!isMoreShow) {
                    valueAnimatorShow.start();
                    isMoreShow = true;
            ...
            case R.id.btnSave:
                if(isEdit){
                    StatService.onEvent(mActivity,"点击借款资产编辑页确认按钮","编辑借款资产",1);
                }else if(isFromAddasset){
                    StatService.onEvent(mActivity,"点击借款资产添加页确认按钮","首次新增借款",1);
                }else{
                    StatService.onEvent(mActivity,"借款列表进入添加页并点击确认按钮","确认新增借款",1);
                }
                if (valid()) {
                    save();
                }
    ...
    void save() {
        ...
        requestModel.setLoanName(cetName.getText().toString());
        requestModel.setAmount(cetMoney.getText().toString());
        requestModel.setLoanStatus(loanStatus);
        ...
        if (isEdit) {
            ...
            mvpPresenter.addloanasset(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        }
    }
    ...
    public void onSuccess(SercetKeyOverdueModel model) {
        if(isEdit) {
            ...
            cetName.postDelayed(new Runnable() {
                @Override
                public void run() {
                    ActivityUtils.startActivity(mActivity,LoanActivity.class);//从详情页跳转过来编辑成功后，回到列表页
                    ActivityUtils.finishActivity(mActivity);
                }
            ...
            cetName.postDelayed(new Runnable() {
                ...
                public void run() {
                    if(isFromList){
                        ...
                        Intent in = new Intent(mActivity, LoanActivity.class);
    ...
    protected void onResume() {
        super.onResume();
        if(isEdit) {
            StatService.onPageStart(mActivity, "编辑借款页面");
        }else{
            StatService.onPageStart(mActivity, "添加借款页面");
        }
    }
    ...
    protected void onPause() {
        ...
        if(isEdit) {
            StatService.onPageEnd(mActivity, "编辑借款页面");
        }else{
            StatService.onPageEnd(mActivity, "添加借款页面");
        }
    }
