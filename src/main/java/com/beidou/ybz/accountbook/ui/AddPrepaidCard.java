 ...
 * module: 添加预付卡
 */
public class AddPrepaidCard extends MvpActivity<CommonPresenter> implements CommonView<SercetKeyOverdueModel>{
    @Bind(R.id.tv_title)
    TextView tvTitle;
    @Bind(R.id.toolbar)
    Toolbar toolbar;
    ...
    private String id, name ,memo, comment ,amount ,from,times;
...
//        tvTitle.setText("添加预付卡");
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StatService.onEvent(mActivity,"点击预付卡资产编辑页返回按钮","放弃预付卡资产编辑",1);
    ...
    private void handleIntent(Intent in){
        ...
        if(comment != null && comment.length() > 0 ){
            cetName.setSelection(comment.length());
        }
        cetTimes.setText(times);
//        tvExpireDate.setText(expireDate);
    ...
    private void bindViewByRxBinding() {
        ...
        Observable<CharSequence> Observable3 = RxTextView.textChanges(cetTimes);
//        Observable<CharSequence> Observable4 = RxTextView.textChanges(tvExpireDate);
    ...
    void save() {
        ...
        requestModel.setComment(cetName.getText().toString());
        requestModel.setAmount(cetMoney.getText().toString());
        requestModel.setTimes(cetTimes.getText().toString());
//        requestModel.setExpireDate(tvExpireDate.getText().toString());
        ...
        if (isEdit) {
            mvpPresenter.updateprepaidcard(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        } else {
            mvpPresenter.addprepaidcard(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        }
    }
    ...
    public void getDataSuccess(SercetKeyOverdueModel model) {
        if(isEdit) {
            ...
            cetName.postDelayed(new Runnable() {
                @Override
                public void run() {
                    ActivityUtils.startActivity(mActivity,PrepaidActivity.class);//从详情页跳转过来编辑成功后，回到列表页
                    ActivityUtils.finishActivity(mActivity);
                }
            ...
            cetName.postDelayed(new Runnable() {
                ...
                public void run() {
                    if(isFromList){
                        ...
                        Intent in = new Intent(mActivity, PrepaidActivity.class);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
//            case R.id.llExpireDate:
//                datePickerDialog.show();
//                break;
            case R.id.llShowMore:
                StatService.onEvent(mActivity,"添加预付卡资产时点击更多","添加更多预付卡信息",1);
                ImeUtil.hideSoftKeyboard(cetName);
                if (!isMoreShow) {
                    valueAnimatorShow.start();
                    isMoreShow = true;
            ...
            case R.id.btnSave:
                if(isEdit){
                    StatService.onEvent(mActivity,"点击预付卡资产编辑页确认按钮","编辑预付卡资产",1);
                }else if(isFromAddasset){
                    StatService.onEvent(mActivity,"点击预付卡资产添加页确认按钮","首次新增预付卡",1);
                }else{
                    StatService.onEvent(mActivity,"预付卡列表进入添加页并点击确认按钮","确认新增预付卡",1);
                }
                if (valid()) {
                    save();
                }
    ...
    protected void onResume() {
        super.onResume();
        if(isEdit) {
            StatService.onPageStart(mActivity, "编辑预付卡页面");
        }else{
            StatService.onPageStart(mActivity, "添加预付卡页面");
        }
    }
    ...
    protected void onPause() {
        ...
        ImeUtil.hideSoftKeyboard(cetName);
        if(isEdit) {
            StatService.onPageEnd(mActivity, "编辑预付卡页面");
        }else{
            StatService.onPageEnd(mActivity, "添加预付卡页面");
        }
    }
