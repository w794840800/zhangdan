...
import com.beidou.ybz.accountbook.adapter.AdapterDomin;
...
/**
 * Author: xu.yang on 2018/1/2
 * QQ:754444814
 * E-mail:754444814@qq.com
 * module: 域名界面
 */
public class DominActivity extends MvpActivity<CommonPresenter> implements CommonView<DominListModel>, OtherView<SercetKeyOverdueModel> {
    ...
    private List<DominListModel.BodyBean.ProListBean> proListBeanList;
    private AdapterDomin mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new AdapterDomin(R.layout.overseaslist_item, null);
        rv.setAdapter(mAdapter);

        tvName.setText("域名名称");
        tvValue.setText("当前估值");
        ...
        mAdapter.setOnItemLongClickListener(new BaseQuickAdapter.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(BaseQuickAdapter adapter, View view, int position) {
                alertDialogUtils.setMessage("确定要删除" + proListBeanList.get(position).getHosturl() + "？");
        ...
        mAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                Intent in = new Intent(mActivity, DominDetailActivity.class);
                in.putExtra("id", proListBeanList.get(position).getId());
                in.putExtra("name", proListBeanList.get(position).getHosturl());
                startActivity(in);
        ...
        alertDialogUtils.setOnDialogClickListener(new DialogClickListener() {
            ...
            public void clickYes(int which) {
                StatService.onEvent(mActivity, "点击域名列表任一删除按钮", "删除域名具体资产", 1);
    ...
    protected void onResume() {
        super.onResume();
        request();
        StatService.onPageStart(mActivity, "域名列表页面");
    }

    @Override
    protected void onPause() {
        super.onPause();
        StatService.onPageEnd(mActivity, "域名列表页面");
    }
    ...
    void request() {
        ...
        mvpPresenter.getdomainlist(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        if (count == 0) {
            mEmptyView.loading();
        }
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tvAddAssets:
                StatService.onEvent(mActivity, "域名列表页点击新增", "新增域名", 1);
                Intent in = new Intent(mActivity, AddDomin.class);
    ...
    @Override
    public void getDataSuccess(DominListModel model) {
        count++;
        try {
            ...
            if (proListBeanList != null && proListBeanList.size() > 0) {
                ...
                mEmptyView.setEmptyText("还没有域名哦～");
                mEmptyView.empty();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    void del(String id) {
        ...
        mvpPresenter.deldomain(encMsg, signMsg, "2", spUtils.getSecretKeyId());
    }
