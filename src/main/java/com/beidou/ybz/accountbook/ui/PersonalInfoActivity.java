...
public class PersonalInfoActivity extends MvpActivity<CommonPresenter> implements CommonView<AccountModel>, OtherView<SercetKeyOverdueModel> {
    ...
    private String wechatName,qqName;
    ...
    public void getDataSuccess(AccountModel model) {
        ...
        if (!portraitUrl.equals("") && !portraitUrl.startsWith("http")) {
            ...
            LogUtils.loge("------getDataSuccess-------"+spUtils.getPortraitUrl());
...
//                .placeholder(R.drawable.defaultavatar)
        ...
        if (model.getBody().getWechatFlag().equals("1")) {
            wechatName = model.getBody().getWechatName();
            tvWechat.setText(wechatName);
        } else {
            tvWechat.setText("未绑定");
        }
        //qq名称
        if (model.getBody().getQqFlag().equals("1")) {
            qqName = model.getBody().getQqName();
            tvQq.setText(qqName);
        } else {
            tvQq.setText("未绑定");
        }
        ...
        realName = model.getBody().getRealName();
        idNo = model.getBody().getIdNo();
        //实名认证信息
...
//        spUtils.setPortraitUrl(model.getBody().getPortraitUrl());
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.rlAuth:
                if(isAuth){
                    Intent in = new Intent(mActivity, AuthSuccessActivity.class);
                    in.putExtra("realName",realName);
                    in.putExtra("idNo",idNo);
                    startActivity(in);
            ...
            case R.id.rel_wechat:
                if (tvWechat.getText().toString().equals("未绑定")) {
                    ...
                    if (spUtils.getIsGesture()) {//如果设置过手势密码
                        ...
                        in.putExtra("thirdName",wechatName);
                        startActivity(in);
                        overridePendingTransition(R.anim.left_in, 0);
