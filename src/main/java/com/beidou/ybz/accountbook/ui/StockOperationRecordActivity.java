 ...
 * module:股票操作记录列表
 */
public class StockOperationRecordActivity extends MvpActivity<CommonPresenter> implements CommonView<StockRecordModel> {
    @Bind(R.id.tv_title)
    TextView tvTitle;
    @Bind(R.id.toolbar)
    Toolbar toolbar;
...
//        requestModel.setId(id);
        ...
        mvpPresenter.getstockrecordlist(encMsg, signMsg, "2", spUtils.getSecretKeyId());
        if(count == 0){
            mEmptyView.loading();
        }
    }
