...
//                .setMessage(dialNumber)
        ...
        etYzm.postDelayed(new Runnable() {
            @Override
            public void run() {
                ImeUtil.showSoftKeyboard(etYzm);
            }
        },300);
            ...
            public void onClick(View view) {
                if (dialog != null & dialog.isShowing()) {
                    ...
                    etYzm.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            ImeUtil.hideSoftKeyboard(etYzm);
                        }
                    },300);
                }
            }
...
//        });
    ...
    void handlerIntent(Intent intent) {
        ...
        } else if(from != null && from.equals("forgetPayPasswdFromWeb")){//from:H5中的忘记支付密码
            isFromForgetPayPasswd = true;
            llDial.setVisibility(View.GONE);
            tvTitle.setText("安全验证");
        }else {
...
//            btnYzm.setClickable(true);
    ...
    public void validSuccess(SercetKeyOverdueModel model) {
        if(isFromForgetPayPasswd){
            ActivityUtils.startActivityRightInWithFrom(mActivity, SetPayPasswdActivity.class,from);
        }else {
